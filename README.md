...
The current analysis revealed how Python coding could be utilized to determine the results of a recent total Congressional election. As described above, the analysis conducted was limited to one particular congressional district consisting of three counties in Colorado, i.e., Jefferson, Denver, and Arapahoe. However, the script has been created to allow for variables that are not "tied" to particular values and operates by generating lists that can be extrapolated from existing data. Therefore, this same exact code has powerful versatility and can be utilized to conduct any election audit analysis that the commission requests:
- The commission can apply the same script to election data files obtained for other congressional district elections in Colorado and yield the same type of data results for each congressional district. Again, because the code was written so that the existing data can be organized into "lists" and "dictionaries", it can be run on any dataset containing candidate names, county names, and ballot ID numbers. 
- Furthermore, the script can also be adapted to audit senatorial race results. An important factor to consider would be at what level the commission wishes to examine datasets, i.e., by counties or by unique voting districts. If the election commission wished to perform analyses on larger datasets at the state level, where all counties in the state of Colorado would be added, we would have to ensure that the dataset included a variable that would identify the voter's precinct, and then this variable would be used in the same manner that "county" was for this election audit. The existing code could be modified to perform this task, where "precinct_options = []" could be used to create a list of precincts, and "precinct_votes" = {}" could create a dictionary for voter turnout by precinct. The same coding used for the county analyses above could be adapted for precinct.    
