...
- Furthermore, the script can also be adapted to audit senatorial race results. An important factor to consider would be at what level the commission wishes to examine datasets, i.e., by counties or by unique voting districts. If the election commission wished to perform analyses on larger datasets at the state level, where all  counties in the state of Colorado would be added, we would have to ensure that the dataset included a variable that would identify the voter's precinct, and then this variable would be used in the same manner that "county" was for this election audit. If the existing code needed to be modified to perform this task, then "precinct_options = []" could be used to create a list of precincts, and "precinct_votes" = {}" could create a dictionary for voter turnout by precinct. The same coding used for the county analyses above could be adapted for precinct.    
